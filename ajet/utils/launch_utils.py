...
def start_ray_service(args, env, cluster=False):
    ...
    if not cluster:
        companion = LaunchCommandWhenAbsent(
            full_argument_list=["ray start --head --block"],
            dir="./",
            tag="ray_service",
            use_pty=True,
        )
        ...
        if HOSTNAME == MASTER_ADDR:
            companion = LaunchCommandWhenAbsent(
                full_argument_list=[
                    f"ray start --head --node-ip-address={MASTER_ADDR} --port={MASTER_PORT} --disable-usage-stats --block"
                ],
                dir="./",
            ...
            companion = LaunchCommandWhenAbsent(
                full_argument_list=[
                    f"ray start --address={MASTER_ADDR}:{MASTER_PORT} --disable-usage-stats --block"
                ],
                dir="./",
